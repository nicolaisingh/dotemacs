...
(use-package agent-shell
  ...
  :init
  (setenv "OPENCODE_CONFIG" (expand-file-name "llm-config/opencode.json" user-emacs-directory))
  (setenv "OPENCODE_CONFIG_DIR" (expand-file-name "llm-config" user-emacs-directory))
  ...
  :config
  (setenv "CONTEXT7_API_KEY" (auth-source-pick-first-password :host "context7"))
  ...
  (ibuffer-saved-filter-groups
   '(("default"
      ...
      ("Scala" (or (mode . scala-mode)
                   (mode . scala-ts-mode)))
      ("JSON" (or (mode . json-mode)
                  (mode . json-ts-mode)))
      ("YAML" (or (mode . yaml-mode)
                  (mode . yaml-ts-mode)))
      ...
      ("Magit" (name . "^magit.*:"))
      ("xref" (name . "^\\*xref\\*$"))
      ("ECA" (or (mode . eca-chat-mode)
                 (mode . eca-mcp-details-mode)
                 (name . "^<eca:")))
      ("emacs" (or
                (name . "^\\*Customize\\*$")
                (name . "^\\*scratch\\*$")
